...
var canvasUtil = (window.canvasUtil = (function () {
    return {
        ...
        screenToCanvas: function (point) {
            var canvasX =
                window.csc * (point.x * canvasUtil.canvasRatio.x) -
                parseInt(window.mc.style.left);
            var canvasY =
                window.csc * (point.y * canvasUtil.canvasRatio.y) -
                ...
                if (distance2 < bothRadii * bothRadii) {
                    if (window.visualDebugging) {
                        var collisionPointCircle = canvasUtil.circle(
                            (circle1.x * circle2.radius +
                                circle2.x * circle1.radius) /
                                bothRadii,
                            (circle1.y * circle2.radius +
...
var bot = (window.bot = (function () {
    return {
        ...
        getCollisionPoints: function () {
            ...
            for (
                var snake = 0, ls = window.snakes.length;
                snake < ls;
                snake++
            ) {
                scPoint = undefined;
        ...
        scoreFood: function (f) {
            f.score =
                Math.pow(
                    Math.ceil(f.sz / bot.opt.foodRoundSize) *
                        bot.opt.foodRoundSize,
                    2
                ) /
                f.distance /
                (Math.ceil(f.da / bot.opt.foodRoundAngle) *
                    bot.opt.foodRoundAngle);
        },
        ...
        every: function () {
            ...
            bot.sectorBoxSide =
                Math.floor(Math.sqrt(window.sectors.length)) *
                window.sector_size;
    ...
    return {
        ...
        onkeydown: function (e) {
            ...
            if (window.playing) {
                ...
                if (e.keyCode === 65) {
                    bot.opt.radiusMult++;
                    window.log("radiusMult set to: " + bot.opt.radiusMult);
                }
        ...
        updateStats: function () {
            ...
            userInterface.overlays.statsOverlay.innerHTML =
                oContent.join("<br/>");
        },
        ...
        onPrefChange: function () {
            ...
            oContent.push(
                "[Y] visual debugging: " + ht(window.visualDebugging)
            );
            ...
            userInterface.overlays.prefOverlay.innerHTML =
                oContent.join("<br/>");
        },
        ...
        onFrameUpdate: function () {
            ...
            userInterface.overlays.botOverlay.innerHTML =
                oContent.join("<br/>");
...
(function () {
    ...
    if (
        ...
        if (
            ...
            window.log(
                "Custom settings found, overwriting current bot options"
            );
        }
    }
